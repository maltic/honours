...
Each selection algorithm was run using these data as input. For every RNA, the single most accurate window size was chosen. Accuracy was judged as the F-Score, which is the harmonic mean of sensitivity and precision (defined in Section \ref{sec:accuracy}).
...
The \texttt{Threshold} function was defined as $\min(\sqrt{ RNALength} \times 9.5, \: 500)$. This formula was found using empirical experimentation. The maximum value of 500 was because only windows up to size 500 were precomputed.
...
A brute force method was used to test a combinatorial set of values for $a$ and $a$ such that $10 \leq a \leq 30$ and $1.5 \leq B \leq 4.0$. In this method all integer values for $a$ were attempted, and values for $b$ were generated with a step size of \texttt{0.1}.
