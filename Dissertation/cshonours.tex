...
The average score between all RNA Interval Selection algorithms was compared (see Table \ref{tab:selectionavg}). The best algorithm appeared to be Top-Down Selection, closely followed by Weighted Activity Selection, then by Bottom-Up Selection. To verify this performance gap, a Wilcoxon Signed Ranks Test was done to compare the recorded F-Scores for each RNA. This test was chosen because the data recorded did not appear to be parametric.


\begin{table}
\centering
\begin{tabular}{l*{6}{c}r}
Selection Method	& Average F-Score  \\
\hline
Weighted Activity Selection 	& 0.703947  \\
Top-Down Selection            	& 0.71684  \\
Bottom-Up Selection           	& 0.381222 \\
Score Selection     	& 0.683729 \\
RNAfold      	& 0.57483 \\
\end{tabular}

\caption{Comparison of RNA Interval Selection algorithms average best F-Score's. RNAfold is also included for comparison.}
\label{tab:selectionavg}
\end{table}

While this test reflected the difference in averages (see Table \ref{tab:wasvstds}), it was not statistically significant ($p = 0.881$). To further test Top-Down Selection and Weighted Activity Selection, another Wilcoxon Signed Ranks Test was done on only RNAs of length $\geq 300$ bases (see Table \ref{tab:wasvstds300}). This revealed a statistically significant difference, indicating that Weighted Activity Selection had higher F-Scores for larger RNAs ($p < 0.001, W = 362$). Finally, a Wilcoxon Signed Ranks Test was done between Weighted Activity Selection and Score Selection to check for circular dominance (see Table \ref{tab:wasvsss}). In accordance with the average population score, Weighted Activity Selection appeared to have higher F-Scores than Score Selection ($p < 0.001, W = 1004.5$).


\begin{table}
\centering
\begin{tabular}{l*{6}{c}r}
Selection Method	& N & Median & Min & Max  \\
\hline
Weighted Activity Selection 	& 392 &	0.757	& 0.0 &	1.0  \\
Top-Down Selection & 392 &	0.8 &	0.0	 & 1.0  \\
\end{tabular}

\caption{Results of a Wilcoxon Signed Ranks Test comparing Weighted Activity Selection and Top-Down Selection ($p = 0.881, W = 2788$).}
\label{tab:wasvstds}
\end{table}
...
Selection Method	& N & Median & Min & Max  \\
\hline
Weighted Activity Selection 	& 68 &	0.585 &	0.39726 &	0.854167  \\
Top-Down Selection & 68	& 0.566 &	0.359511 &	0.854167  \\
\end{tabular}

\caption{Results of a Wilcoxon Signed Ranks Test comparing Weighted Activity Selection and Top-Down Selection for RNAs with length $\geq 300$ bases ($p < 0.001, W = 362$).}
\label{tab:wasvstds300}
\end{table}
...
\hline
Weighted Activity Selection 	& 392 &	0.757 &	0.0 &	1.0  \\
Score Selection & 392 &	0.731 &	0.0 &	1.0  \\
\end{tabular}

\caption{Results of a Wilcoxon Signed Ranks Test comparing Weighted Activity Selection and Score Selection ($p < 0.001, W = 1004.5$).}
\label{tab:wasvsss}
\end{table}

A Wilcoxon Signed Ranks Test was also used to compare the best recorded F-Scores for Weighted Activity Selection, and those recorded for RNAfold. 


Instead of having lots of tables, lets just have one table with all the tests as separate rows.

Finish off with a graph showing the accuracy of WAS vs Zukers.
